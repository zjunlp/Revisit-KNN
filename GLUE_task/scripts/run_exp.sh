...
# TASK: SST-5 / trec / MNLI / QNLI / boolq / cb
...
case $TASK in
    ...
    trec)
        ...
        ;;
    boolq)
        TEMPLATE=*cls*question:_*sent_1*?*mask*.*sent_0**sep+*
        MAPPING="{0:'False',1:'True'}"
        TASK_EXTRA="--max_seq_len 512"
        ;;
    cb)
        TEMPLATE=*cls**sent_1*?*mask*.*sent_0**sep+*
...
OUTPUT_DIR=ckpt/pt_16shot/$TASK-$K-$SEED/$REAL_BS-$LR
